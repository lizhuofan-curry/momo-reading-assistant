...
if (!await videoPage.getByRole("heading", { name: /让视频里的声音和字幕/ }).isVisible()) throw new Error("视频英文页面未显示");
...
await videoPage.locator("#recognize-video").click();
await videoPage.getByText(/视频英文提取完成/).first().waitFor({ state: "visible", timeout: 15000 });
...
if (!await videoMobile.getByRole("button", { name: /开始提取视频英文/ }).isVisible()) throw new Error("移动端视频识别按钮未显示");
